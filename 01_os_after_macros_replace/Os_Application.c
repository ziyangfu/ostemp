 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline Os_StdReturnType   Os_AppCheckRestartOption (
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline Os_StdReturnType   Os_AppCheckIsUserApp (
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline Os_StdReturnType   Os_AppCheckMayTerminate (
  const   Os_AppConfigType OS_CONST * TerminatorApp,
  const   Os_AppConfigType OS_CONST * TargetApp
);
 ...
 *  If the ObjectType is invalid or has no application association, ((void *)0) is returned.
 *  If the ObjectType is valid, but the ObjectID is out of range, ((void *)0) is returned.
 *
 ...
 *  \retval         ((void *)0)      If the given ObjectType or ObjectID is invalid.
 *
 *  \context        OS_INTERNAL
 *
 *  \reentrant      TRUE
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline const   Os_AppConfigType OS_CONST *   Os_AppObjectGetApp (
 ...
 *  \pre            -
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline Os_StdReturnType   Os_AppObjectAccessIsAllowed (
 ...
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline void   Os_AppKillMembers (
  const   Os_AppConfigType OS_CONST * App
);
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
OS_ALWAYS_INLINE static inline void   Os_AppKillInterruptedMemberThreads (
  const   Os_AppConfigType OS_CONST * App
);
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
static void     Os_AppKillMemberThread
(
 ...
 *  \pre            Concurrent access to given object is prevented by caller.
 **********************************************************************************************************************/
static void     Os_AppKillInternal
(
  const   Os_AppConfigType OS_CONST * App,
  RestartType RestartOption
);
 ...
 *  \pre            -
 **********************************************************************************************************************/
static Os_StatusType     Os_AppTerminateApplication
(
  const   Os_AppConfigType OS_CONST * App,
  RestartType RestartOption,
  const   Os_AppConfigType OS_CONST * CallerCurrentApplication
);
...
{
  const   Os_AppConfigType OS_CONST * ownerApp;
 ...
 *
 */
static inline void     Os_AppKillInternal
(
  const   Os_AppConfigType OS_CONST * App,
  RestartType RestartOption
)
{
  ...
  if(RestartOption == RESTART)
  {
    ...
    Os_Assert((Os_StdReturnType)(App->RestartTask != ((void *)0)));                                                        /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */
 ...
 *
 */
ApplicationType     Os_Api_GetApplicationID
(
  void
)
{
 ...
 *
 */
ApplicationType     Os_Api_GetCurrentApplicationID
(
  void
)
{
 ...
 *
 */
Os_StatusType     Os_Api_GetApplicationState
(
 ...
 *
 */
ObjectAccessType     Os_Api_CheckObjectAccess
(
  ApplicationType ApplID,
  ObjectTypeType ObjectType,
  Os_ObjectIdType ObjectID
 ...
 *
 */
ApplicationType     Os_Api_CheckObjectOwnership
(
  ObjectTypeType ObjectType,
  Os_ObjectIdType ObjectID
)
{
  ...
  /* #20 If the reference is a ((void *)0) the result is INVALID_OSAPPLICATION. */
  if(OS_UNLIKELY(ownerApp == ((void *)0)))
  {
 ...
 *
 */
Os_StatusType     Os_Api_AllowAccess
(
  void
)
{
 ...
 *
 */
Os_StatusType     Os_Api_TerminateApplication
(
  ApplicationType Application,
  RestartType RestartOption
)
...
)
{
  ...
  else if(OS_UNLIKELY((RestartOption == RESTART) && (App->RestartTask == ((void *)0))))
  {
 ...
 *
 */
Os_StatusType     Os_AppTerminateApplicationLocal                                                            /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
(
  const   Os_AppConfigType OS_CONST * App,
  RestartType RestartOption,
  ApplicationType CurrentCallerApplicationID
 ...
 **********************************************************************************************************************/
Os_StatusType     Os_AppGetApplicationStateLocal                                                             /* COV_OS_HALPLATFORMTESTEDASMULTICORE */
(
  const   Os_AppConfigType OS_CONST * App,
  ApplicationStateRefType Value
)
...
\ID SBSW_OS_APPGETID_002
 \DESCRIPTION    Os_AppGetId is called with the return value of Os_AppObjectGetApp(). That function always returns a
                 valid pointer or ((void *)0). Check against ((void *)0) exists.
...
\CM CM_OS_APPDYN_M
      Verify that the Dyn pointer of each application is no ((void *)0). An initialization of this pointer
      with an incorrect type is expected to be checked by the compiler.

\CM CM_OS_APPGETCORE_M
      Verify that the Core reference in each application instance is no ((void *)0).
...
\CM CM_OS_APPGETSTARTUPHOOK_N
      ...
      The caller ensures, that the returned pointer is not used for write accesses, if the pointer is ((void *)0).
...
\CM CM_OS_APPID2APP_M
      Verify that:
        1. each Application in OsCfg_AppRefs except the last is a non ((void *)0),
...
\CM CM_OS_APP_ALARMREF_M
      Verify that:
        1. each application's AlarmRefs pointer is no ((void *)0),
        2. each application's AlarmRefs size is equal to the AlarmCount + 1 and
        3. each element in AlarmRefs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_COUNTERREF_M
      Verify that:
        1. each application's CounterRefs pointer is no ((void *)0),
        2. each application's CounterRefs size is equal to the CounterCount + 1 and
        3. each element in CounterRefs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_ISRCAT1REF_M
      Verify that:
        1. each application's IsrCat1Refs pointer is no ((void *)0),
        2. each application's IsrCat1Refs size is equal to the IsrCat1Count + 1 and
        3. each element in IsrCat1Refs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_ISRCAT2REF_M
      Verify that:
        1. each application's IsrCat2Refs pointer is no ((void *)0),
        2. each application's IsrCat2Refs size is equal to the IsrCat2Count + 1 and
        3. each element in IsrCat2Refs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_SCHTREF_M
      Verify that:
        1. Verify that each application's SchTRefs pointer is no ((void *)0),
        2. each application's SchTRefs size is equal to the SchTCount + 1 and
        3. each element in SchTRefs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_TASKREF_M
      Verify that:
        1. each application's TaskRefs pointer is no ((void *)0),
        2. each application's TaskRefs size is equal to the TaskCount + 1 and
        3. each element in TaskRefs of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).

\CM CM_OS_APP_SERIVCEFUNCTION_M
      Verify that:
        1. each application's ServiceFunctions pointer is no ((void *)0),
        2. that each application's ServiceFunctions size is equal to the ServiceFunctionCount + 1 and
        3. each element in ServiceFunctions of each application except the last one is no ((void *)0), the last one must be
           ((void *)0).
*/
