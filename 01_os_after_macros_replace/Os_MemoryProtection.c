...
\CM CM_OS_MPSYSTEM_HWCONFIG_M
      Verify that the HwConfig reference in each Os_MpSystemConfigType instance is a non ((void *)0).

\CM CM_OS_MPCORE_HWCONFIG_M
      Verify that the HwConfig reference in each Os_MpCoreConfigType instance is a non ((void *)0).

\CM CM_OS_MPACCESSRIGHTS_APPACCESSRIGHTS_M
      Verify that the AppAccessRights reference in each Os_MpAccessRightsType instance is a non ((void *)0).

\CM CM_OS_MPACCESSRIGHTS_THREADACCESSRIGHTS_M
      Verify that the ThreadAccessRights reference in each Os_MpAccessRightsType instance is a non ((void *)0).
