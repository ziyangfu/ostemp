...
#define OS_SERVICECALL_LIST_END            ((  Os_ServiceCallType OS_VAR_NOINIT *)(((void *)0)))
...
struct Os_ServiceCallType_Tag
{
  ...
  const   Os_AppConfigType OS_CONST * PrevApplication;

  /*! Memory protection settings before the service call. */
  const   Os_MpAccessRightsType OS_CONST * PrevMpAccessRights;

  /*! List of services before the service call. */
    Os_ServiceCallType OS_VAR_NOINIT * PrevServiceCalls;

  /*! Context before service call. */
  Os_Hal_ContextType CallerContext;

  /*! Context of the current service call. */
    Os_Hal_ContextType OS_VAR_NOINIT * ServiceCallContext;
...
typedef P2void    , Os_ServiceTfCbkType
(
...
typedef P2void    , Os_ServiceNtfCbkType
(
...
struct  Os_ServiceStackConfigType_Tag
{
  ...
  const   Os_Hal_ContextStackConfigType OS_CONST * HwConfig;
...
struct Os_ServiceStackPoolConfigType_Tag
{
  /*! Dynamic data of the stack pool. */
    Os_ServiceStackPoolType OS_VAR_NOINIT * Dyn;

  /*! References to free stacks. */
  Os_BitArrayConfigType AvailabilityMap;

  /*! Pointer to stacks. */
  const   Os_ServiceStackConfigType OS_CONST * Stacks;
...
/*! Type of user provided trusted functions. */
typedef P2void    , Os_ServiceFtfCbkType
(
 ...
 *  \pre            Current thread is in service function.
 **********************************************************************************************************************/
OS_NORETURN void   Os_ServiceNtfWrapper (
  const   Os_ServiceCallType OS_VAR_NOINIT * ServiceCall
);
