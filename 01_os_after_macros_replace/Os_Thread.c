...
\CM CM_OS_THREADGETTRACE_M
      Verify that the Trace reference in each thread instance is no ((void *)0), if ORTI or TimingHook tracing is enabled.

\CM CM_OS_THREADGETSTACK_M
      Verify that the Stack reference of each thread is no ((void *)0).

\CM CM_OS_THREADGETINTERRUPTAPISTATE_M
      Verify that the IntApiState reference in each thread instance is no ((void *)0).

\CM CM_OS_THREADGETCORE_M
      Verify that the Core reference in each thread instance is no ((void *)0).

\CM CM_OS_THREADGETCONTEXT_M
      Verify that the Context reference in each thread instance is no ((void *)0).

\CM CM_OS_THREADGETCONTEXTCONFIG_M
      Verify that the ContextConfig reference in each thread instance is no ((void *)0).

\CM CM_OS_THREADGETOWNERAPPLICATION_M
      Verify that the OwnerApplication reference in each thread instance is no ((void *)0).
...
\CM CM_OS_THREADGETMPACCESSRIGHTSINITIAL_M
      Verify that the MpAccessRightsInitial reference in each thread instance is no ((void *)0), if memory protection
      is enabled.
...
\CM CM_OS_THREADGETTPCONFIG1_M
      Verify that the TimingProtection reference in each thread instance is a non-((void *)0) if timing protection is
      enabled for the owner core. As a result, the timing protection references in thread and owner core need
      to be both ((void *)0) or both non-((void *)0).
...
\CM CM_OS_THREADGETACCESSRIGHTS_M
      Verify that the AccessRights reference in each thread instance is no ((void *)0).
...
\CM CM_OS_THREADPRETHREAD_CALLBACK_M
      ...
      ((void *)0) for all Thread objects defined in other types.

\CM CM_OS_THREADPRETHREAD_R
      Implementation ensures that the function pointer is not ((void *)0) before calling it.

\CM CM_OS_THREADGETFPUCONTEXT_M
      Verify that
        - FpuContext is no ((void *)0) in case that Context saving is enabled for thread (UsesFpu).
